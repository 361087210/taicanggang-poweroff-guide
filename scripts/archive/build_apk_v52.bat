...
echo ============================================
echo Cordova Android APK Build Script V5.2
echo (含原生社交分享插件: 微信/QQ/钉钉)
echo ============================================
echo.
...
echo Syncing V5.2 files to Cordova www...
echo.
...
echo [OK] Copied config.xml (含 socialsharing 插件声明)
...
REM ===== Ensure plugins (V5.2: socialsharing 新增) =====
echo Checking plugins...
if not exist "plugins\cordova-plugin-x-socialsharing" (
    echo Adding cordova-plugin-x-socialsharing...
    call node_modules\.bin\cordova.cmd plugin add cordova-plugin-x-socialsharing
    if errorlevel 1 (
        echo WARNING: socialsharing plugin add failed - retry via config.xml prepare
    )
)
echo.
...
if exist "%APK_PATH%" (
    ...
    copy /Y "%APK_PATH%" "%~dp0release\太仓港断电指导V5.2.apk" >nul
    echo APK saved as: release\太仓港断电指导V5.2.apk
) else (
    echo.
...
echo.
echo V5.2 Build process complete.
