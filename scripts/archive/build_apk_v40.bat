...
echo ============================================
echo Cordova Android APK Build Script V4.0
echo ============================================
echo.
...
echo Syncing V4.0 files to Cordova www...
echo.

REM Copy demo.html as index.html
...
REM Copy vehicles_data.js if exists
...
REM Copy vehicle_images if not already synced
if exist "%~dp0vehicle_images" (
    if not exist "%~dp0tcg_app\www\vehicle_images" (
...
if exist "%APK_PATH%" (
    ...
    REM Copy to project root with version name
    copy /Y "%APK_PATH%" "%~dp0太仓港断电指导V4.0.apk" >nul
    echo APK saved as: 太仓港断电指导V4.0.apk
) else (
    echo.
...
echo.
echo V4.0 Build process complete.
