...
function _install(){
  ...
  window.pullApprovedStatusFromFeishu=async function(userParam,fullMerge){
    ...
    try{
      ...
      if(!me){
        /* P0 迁移缺口: 未命中连接键。网页端镜像不含明文手机号, 因而**无法**区分
         * "未注册"与"存量账号从未在 P0(>=10.19.2) 版 App 登录过→云端镜像缺 linkKey"。
         * 注: v10.19.1 不含 P0(仍是旧的手机号哈希方案), 只有 >=10.19.2 的 App 才派生 linkKey。
         * 此处**故意不携带任何可区分账号存在性的信号**, 由 doLogin 对所有网页登录失败
         * 统一给出同一条可操作引导(去 App 完成一次登录再重试)。
         * 这是**隐私选择**: 任何"仅对已存在账号才提示 / 点明未完成升级"的收窄, 都会
         * 泄露该手机号是否为已注册账号, 违背 P0「不公开账号表」的初衷。 */
        return false;
      }
