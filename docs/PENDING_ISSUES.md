...
> 原因（自举顺序）：门禁实跑证明**当前 `main` 自身即处于 FAIL**（`44fb793` 进了 main 但版本仍是 10.19.2）。
> 若现在合入，CI 从第一天就是红的 → **红着的门禁一定会被人关掉**，那还不如不做。
> 正确路径：**随 10.19.3（或并入 10.20.0）一起落地** —— 发布会在 main 上把版本升到位并打 tag，
> 届时 `merge-base(TAG,HEAD)..HEAD` 为空 → 门禁 PASS，**干净落地、无需任何旁路开关**。
> **明确拒绝**"先加 WARN-only 环境变量绕过 FAIL"的做法：那是给门禁装旁路，且旁路一定会被忘记移除。
> **宁可等一次发布，也不要一个可绕过的门禁。**
