...
| 版本 | 支持状态 | 说明 |
| ---- | -------- | ---- |
| V5.7.x | ✅ 支持 | 当前活跃版本，含 V5.7 混淆凭证 + V5.4 密码哈希加固 |
| V5.3.x / V5.4.x | ✅ 支持 | 历史活跃版本 |
...
### 飞书 App Secret
- **混淆内置（V5.7）**：默认凭证以 `_fsDec` 混淆存储，开箱即用，非明文合规
- **可覆盖**：可在「设置 → 飞书配置」手动填写覆盖，保存于 `localStorage`
- **代码库零明文**：源码、文档、测试脚本中均不含明文凭证
- **测试脚本**：通过环境变量 `TCG_FEISHU_APP_SECRET` 传递
- **V5.8 规划**：Secret 服务端化（云函数代理），APP 仅持设备码
...
| 客户端直连飞书 | App Secret 存在于客户端（V5.7 起 `_fsDec` 混淆），APK 反编译可提取 | **V5.8 ✅ 规划：服务端代理（云函数）** |
| 无传输加密 | 数据上传下载走 HTTPS，但本地存储无加密 | V5.8+（评估中） |
