...
| 版本 | 支持状态 | 说明 |
| ---- | -------- | ---- |
| **10.15.4** | ✅ 支持 | **当前活跃版本**（V11.3 里程碑）：问题反馈修复版（批量Word含图/板块选项细化/照片压缩质量提升）+ 真实 AI 反馈分析 + 飞书群通知闭环 + 审计日志/密钥加密/R8 加固 |
| V10.x | ✅ 支持 | 历史活跃版本（数据分仓 + 零配置同步），建议升级 10.15.4 |
| V5.x | ❌ 不再支持 | 历史版本，存在飞书 CORS / 明文凭证等遗留问题，建议升级 10.15.4 |

## 凭证安全（V11.3 里程碑，随 10.15.4 交付）
